...
import "../vendor/libraries/ABDKMath64x64.sol";

library Perlin {
    uint16 constant vecsDenom = 1000;
    uint16 constant perlinMax = 64;

    // Perlin Noise
    // interpolation function [0,1] -> [0,1]
    function smoothStep(int128 x) internal pure returns (int128) {
        return x;
    }
    ...
    function getGradientAt(
        ...
        uint32 scale,
        uint32 seed
    ) internal pure returns (int16[2] memory) {
    ...
    function getCorners(
        ...
        uint32 y,
        uint32 scale
    ) internal pure returns (uint32[2][4] memory) {
    ...
    function getSingleScalePerlin(
        ...
        return
            ABDKMath64x64.divi(
                int256(resNumerator),
                int256(uint256(vecsDenom)) * int256(int32(scale))**3
            );
    }
